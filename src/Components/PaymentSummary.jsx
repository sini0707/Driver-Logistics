...
const PaymentSummary = ({ orders, actualWeight }) => {
 ...
 const totalWeight = orders.reduce((total, order) => {
  ...
  const calculateSubTotal = () => {
    const baseFreight = 10.00;
    const docketCharge = 2016.00;
    const stationaryCharge = 50.00;
    const handlingCharge = 500.00;
    const cgst = 200.00;
    const sgst = 200.00;
    const igst = 0.00;

    const subTotal = baseFreight + 
                    docketCharge + 
                    stationaryCharge + 
                    handlingCharge + 
                    cgst + 
  ...
  >
    ...
    <div className="bg-gray-50 p-4 rounded-lg flex justify-between items-center">
        ...
        <span className="text-gray-900">${totalWeight}</span>
      </div>
